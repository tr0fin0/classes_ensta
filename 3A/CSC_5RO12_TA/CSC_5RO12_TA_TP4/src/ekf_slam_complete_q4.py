...
Q_sim = (1 * np.diag([0.1, np.deg2rad(1)])) ** 2
...
def jacob_motion(x, u):
    ...
    A = np.array([[1.0, 0.0, float(-DT * u[0] * math.sin(x[2, 0]))],
...
def jacob_augment(x, y):
    ...
    Jr = np.array([[1.0, 0.0, -y[0] * math.sin(x[2] + y[1])],
                   [0.0, 1.0, y[0] * math.cos(x[2] + y[1])]])

    Jy = np.array([[math.cos(x[2] + y[1]), -y[0] * math.sin(x[2] + y[1])],
                   [math.sin(x[2] + y[1]), y[0] * math.cos(x[2] + y[1])]])
