function [ total_error ] = BicycleToPath( K_rho, K_alpha, rho_threshold )
    ...
    while max(abs(dist(xTrue(1:2,1),Path(1:2,end))))>.005 && k<20000
        % Compute Control
        u = BicycleToPathControl( xTrue, Path, K_rho, K_alpha, rho_threshold );
    ...
    total_error = sum(XErrStore(1,:));
end;
disp([k, ',',num2str(total_error)]);
