...
{ 
    ...
    int world_size, rank;
    // int data_size, *result, *results;
    int data_size, *results;
    ...
    int job_width = W / world_size;
    if (W % world_size) job_width += 1;
    data_size = job_width * H;
    // result = new int [data_size];


    const int maxIter = 8*65536;
    auto iters = computeMandelbrotSet( W/world_size, H, maxIter );
    savePicture("mandelbrot_parallel.tga", W, H, iters, maxIter);

    if (rank == 0) results = new int [W * H];
    MPI_Gather(iters, data_size, MPI_INT, results, data_size, MPI_INT, MASTER, MPI_COMM_WORLD);
