...
class BBNode:
    ...
    - isStandard: True si on considère un problème standard
    """

    @classmethod
    def create_root(cls, A, rhs, obj, isMinimization, isStandard):
        return cls(A=A,
                   rhs=rhs,
                   obj=obj,
    ...
    """
     Résout la relaxation linéaire du noeud "self" et branche si nécessaire (position.e. si la relaxation fournit une solution fractionnaire)
    ...
    def branch(self, tree):
        ...
        # print(self.tableau.A)
        # self.tableau.addSlackAndSolve()
        ...
        # II
        def createBranch(value: float, position: int, direction: int):
            if direction == +1:
                roundValue = +1 * math.ceil(value)

            elif direction == -1:
                roundValue = -1 * math.floor(value)

            else:
                print(f'error: direction {direction} is not valide')

            newCondition = np.zeros_like(self.tableau.bestSolution)
            newCondition[position] = direction

            return BBNode.create_non_root(self, newCondition, roundValue)

        if isFractional(self.tableau.bestObjective):

            # find the first fractional value in the best solutions array of the linear relaxation solution
            position = 0
            for solution in self.tableau.bestSolution:

                if isFractional(solution):
                    # creates superior and inferior branches
                    sup = createBranch(solution, position, +1)
                    inf = createBranch(solution, position, -1)
                    break
            ...
            sup.tableau.applySimplexPhase1And2()
            inf.tableau.applySimplexPhase1And2()

            # TODO: fix loop bug
            # by trying to branch the codes runs in a infinite loop because the applySimplexPhase1And2 method does not correctly solves the problem with the conditions added
    ...
    def __init__(self,
        ...
        if parent == None:
            ...
            self.tableau = tableau.Tableau(newMA, newMRhs, parent.tableau.c,
                                           parent.tableau.isMinimization,
                                           parent.tableau.isStandard)
